...
function getPath(points) {
  ...
  for (let i = 4; i < points.length; i += 2) {
    str += `S ${points[i][0]} ${points[i][1]}, ${points[i + 1][0]} ${points[i + 1][1]}`;
  }
  str += "Z";
  return str;
}
...
function getPoints(n_points, center, radius) {
  ...
  let regPoints = [[center[0], center[1] - radius], [center[0] + mag, center[1] - radius]];
...
function refreshPoints(points, deltas, startTimestamp, timestamp, endTimestamp) {
...
export function Blob() {
  const n_points = 5, center = [500, 500], radius = 400;
  const [curPoints, setCurPoints] = useState(getPoints(n_points, center, radius));
  ...
  const animStep = (timestamp) => {
    ...
    setCurPoints(points => refreshPoints(points, deltasRef.current,
      startTimestampRef.current, timestamp, nextTimestampRef.current));
  ...
  return <svg id="blob" viewBox="0 0 1000 1000 " xmlns="http://www.w3.org/2000/svg">
    <path
      fill="#feb12f"
      d={getPath(curPoints)}
    ></path>
